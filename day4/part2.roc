...
processDataHelper = \{seen, remaining}, boardsData ->
    nextBoardsData = List.dropIf boardsData (\boardData -> hasBingo boardData.rowscols seen)
    if List.isEmpty nextBoardsData then
        boardData = List.first boardsData |> Result.withDefault {board: [], rowscols: []}
        ...
        when List.first remaining is
            Ok next ->
                processDataHelper {seen: List.append seen next, remaining: List.dropFirst remaining} nextBoardsData
            Err _ ->
